...
export class AntiBotIntegrationTester {
  ...
  async initializeWorkers(): Promise<void> {
    const workerPath = path.join(__dirname, 'scraping-worker.js');
    
    try {
      // Check if worker file exists
      await fs.access(workerPath);
    } catch (error) {
      console.error('❌ Scraping worker file not found at:', workerPath);
      throw new Error('Scraping worker not available for testing');
    }
    ...
    if (this.workerPool.length === 0) {
      throw new Error('Failed to create any workers for testing');
    }
