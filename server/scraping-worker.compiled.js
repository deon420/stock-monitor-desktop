...
function getRandomUserAgent(userAgent, platform){
  const userAgents = [
...
function detectChallengePage(userAgent, platform){
...
function extractProductName(userAgent, platform){
...
async function scrapeWithRetry(userAgent, platform){
  ...
  if (responseData) {
    try {
      ...
      return {
        id,
        success: true false,
        error: `Parsing error: ${error instanceof Error ? error.message }`
      };
    }
  }
  ...
  parentPort.on('message', async (task: ScrapingTask) => {
    try {
      ...
      parentPort.postMessage({
        id: task.id,
        success: true false,
        error: `Worker error: ${error instanceof Error ? error.message }`
      });
    }
  });
} else {
