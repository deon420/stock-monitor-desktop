...
const workerAxios = axios.create({
  ...
  httpAgent: new HttpAgent({ keepAlive: true, maxSockets: 5 }),
  httpsAgent: new HttpsAgent({ keepAlive: true, maxSockets: 5 })
});
...
function generateBrowserHeaders(userAgent, platform) {
  const baseHeaders = {
...
function detectAntiBot(html, url, responseCode, responseTime, headers) {
  ...
  if (cloudflareDetection.confidence > result.confidence) {
  ...
  if (wafDetection.confidence > result.confidence) {
  ...
  if (rateLimitDetection.confidence > result.confidence) {
  ...
  if (ipBlockDetection.confidence > result.confidence) {
  ...
  if (jsDetection.confidence > result.confidence) {
  ...
  if (platformDetection.confidence > result.confidence) {
  ...
  if (anomalyDetection.confidence > result.confidence) {
...
function detectCloudflareProtection(html, headers) {
  const lowerHtml = html.toLowerCase();
  let confidence = 0;
  const details = {};

  // Check for Cloudflare headers
  if (headers['cf-ray'] || (headers['server'] && headers['server'].includes('cloudflare'))) {
    confidence += 0.2;
...
// 2. AWS WAF Detection
function detectAWSWAF(html, headers, responseCode) {
  const lowerHtml = html.toLowerCase();
  let confidence = 0;
  const details = {};
...
// 3. Rate Limiting Detection
function detectRateLimiting(html, headers, responseCode, responseTime) {
  const lowerHtml = html.toLowerCase();
  let confidence = 0;
  const details = {};
...
// 4. IP Block Detection
function detectIPBlock(html, headers, responseCode) {
  const lowerHtml = html.toLowerCase();
  let confidence = 0;
  const details = {};
...
// 5. JavaScript Challenge Detection
function detectJavaScriptChallenge(html) {
  const lowerHtml = html.toLowerCase();
  let confidence = 0;
  const details = {};
  ...
  if (html.includes('<script>') && html.replace(/<script[^>]*>.*?<\/script>/g, '').trim().length < 500) {
    confidence += 0.3;
...
// 6. Platform-specific Detection
function detectPlatformSpecificBlocking(html, platform, headers) {
  const lowerHtml = html.toLowerCase();
  let confidence = 0;
...
// 7. Response Anomaly Detection
function detectResponseAnomalies(html, responseCode, responseTime, url) {
  let confidence = 0;
  const details = {};
...
function extractProductName(html, platform) {
...
async function scrapeWithRetry(task) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const antiBot = detectAntiBot(response.data, url, response.status, requestTime, response.headers || {});
